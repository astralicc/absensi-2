...
<head>
    ...
    <title>Detail Anak - {{ $child->name }} - Absensi SMKN 12 Jakarta</title>
    ...
    <link rel="stylesheet" href="{{ asset('css/parents/detailAnak.css') }}">
</head>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <div class="flex-1 flex flex-col md:ml-64">
            ...
            <header
                ...
                <!-- Page Title -->
                <h1 class="text-xl font-semibold text-gray-900 dark:text-white">Detail Anak</h1>
            ...
            <main class="flex-1 overflow-y-auto p-6">
                ...
                <a href="{{ route('children.data') }}"
                    ...
                    <i class="fa-solid fa-arrow-left"></i> Kembali ke Data Anak
                </a>
                ...
                <div
                    ...
                    <div class="flex items-start gap-6">
                        <div
                            class="h-24 w-24 rounded-2xl bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white text-4xl font-bold shadow-lg">
                            {{ strtoupper(substr($child->name, 0, 1)) }}
                        </div>
                        <div class="flex-1">
                            <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-1">{{ $child->name }}</h2>
                            <p class="text-gray-500 dark:text-gray-400 mb-1">NIS: {{ $child->id }}</p>
                            <p class="text-gray-500 dark:text-gray-400">{{ $child->email }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div
                    class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Statistik Kehadiran Tahun
                        {{ date('Y') }}</h3>
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        @foreach ($monthlyStats as $stats)
                            ...
                            @endphp
                            <div
                                class="p-4 rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-700">
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
                                    {{ $stats['monthName'] }}</p>
                                <p
                                    class="text-3xl font-bold text-{{ $color }}-600 dark:text-{{ $color }}-400 mb-1">
                                    {{ $rate }}%</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                                    {{ $stats['presentDays'] }}/{{ $stats['totalDays'] }} hari</p>
                                @if ($stats['lateDays'] > 0)
                                    <p class="text-xs text-yellow-600 mt-1">{{ $stats['lateDays'] }} kali terlambat</p>
                                @endif
                            </div>
                        @endforeach
                    </div>
...
<script src="{{ asset('js/child/detailAnak.js') }}"></script>
